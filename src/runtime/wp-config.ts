import { cpSync, existsSync, lstatSync, mkdirSync, rmSync, symlinkSync, writeFileSync } from 'node:fs';
import { dirname, join, relative, resolve } from 'node:path';
import { randomBytes } from 'node:crypto';
import { fileURLToPath } from 'node:url';
import type { LoadedAstroPressConfig } from '../config.js';

export function writeWordPressConfig(config: LoadedAstroPressConfig) {
  const docroot = resolve(config.root, config.wordpress.docroot);
  const contentDir = resolve(config.root, config.wordpress.contentDir);
  const wpConfigPath = join(docroot, 'wp-config.php');
  const publicUrl = config.wordpress.url;

  mkdirSync(docroot, { recursive: true });
  mkdirSync(join(contentDir, 'plugins'), { recursive: true });
  mkdirSync(join(contentDir, 'mu-plugins'), { recursive: true });
  mkdirSync(join(contentDir, 'uploads'), { recursive: true });
  ensureDefaultContentFiles(contentDir);
  ensureContentSymlink(docroot, contentDir);

  writeFileSync(wpConfigPath, renderWpConfig(config, publicUrl, contentDir), 'utf8');
  console.log(`✓ wp-config.php generated at ${relative(config.root, wpConfigPath)}`);
}

export function phpServerUrl(config: LoadedAstroPressConfig) {
  return `http://${config.dev.phpHost}:${config.dev.phpPort}`;
}

function renderWpConfig(config: LoadedAstroPressConfig, publicUrl: string, contentDir: string) {
  const database = config.database;

  return `<?php
/**
 * Generated by AstroPress for local development.
 * Do not edit this file by hand; update astropress.config.ts or environment variables instead.
 */

define('DB_NAME', ${phpString(database.name)});
define('DB_USER', ${phpString(database.user)});
define('DB_PASSWORD', ${phpString(database.password)});
define('DB_HOST', ${phpString(`${database.host}:${database.port}`)});
define('DB_CHARSET', 'utf8mb4');
define('DB_COLLATE', '');

${authKeys()}

$table_prefix = ${phpString(database.tablePrefix)};

define('WP_DEBUG', true);
define('WP_DEBUG_DISPLAY', false);
define('WP_DEBUG_LOG', true);
@ini_set('display_errors', '0');
define('WP_ENVIRONMENT_TYPE', 'local');
define('WP_DEFAULT_THEME', 'astropress');
define('WP_HOME', ${phpString(publicUrl)});
define('WP_SITEURL', ${phpString(publicUrl)});
define('WP_CONTENT_DIR', ${phpString(contentDir)});
define('WP_CONTENT_URL', WP_HOME . '/wp-content');
define('ASTROPRESS_ROOT', ${phpString(config.root)});
define('ASTROPRESS_INTERNAL_SECRET', ${phpString(process.env.ASTROPRESS_INTERNAL_SECRET ?? '')});
define('ASTROPRESS_ASSETS_MANIFEST', ${phpString(join(config.root, config.blocks.outDir, 'astropress-manifest.json'))});
define('ASTROPRESS_MENUS', ${phpString(JSON.stringify(config.wordpress.menus))});
define('ASTROPRESS_OMIT_DEFAULT_ASSETS', ${config.wordpress.omitDefaultAssets ? 'true' : 'false'});

if (
    (isset($_SERVER['HTTP_X_FORWARDED_PROTO']) && str_contains($_SERVER['HTTP_X_FORWARDED_PROTO'], 'https'))
    || (isset($_SERVER['HTTP_X_FORWARDED_SSL']) && $_SERVER['HTTP_X_FORWARDED_SSL'] === 'on')
) {
    $_SERVER['HTTPS'] = 'on';
}
if (isset($_SERVER['HTTP_X_FORWARDED_HOST'])) {
    $_SERVER['HTTP_HOST'] = $_SERVER['HTTP_X_FORWARDED_HOST'];
}
if (isset($_SERVER['HTTP_X_FORWARDED_PORT'])) {
    $_SERVER['SERVER_PORT'] = $_SERVER['HTTP_X_FORWARDED_PORT'];
} elseif (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on') {
    $_SERVER['SERVER_PORT'] = '443';
}

if (! defined('ABSPATH')) {
    define('ABSPATH', __DIR__ . '/');
}

require_once ABSPATH . 'wp-settings.php';
`;
}

function authKeys() {
  const names = [
    'AUTH_KEY',
    'SECURE_AUTH_KEY',
    'LOGGED_IN_KEY',
    'NONCE_KEY',
    'AUTH_SALT',
    'SECURE_AUTH_SALT',
    'LOGGED_IN_SALT',
    'NONCE_SALT',
  ];

  return names.map((name) => `define('${name}', '${randomBytes(32).toString('base64')}');`).join('\n');
}

function phpString(value: string) {
  return `'${value.replace(/\\/g, '\\\\').replace(/'/g, "\\'")}'`;
}

function ensureContentSymlink(docroot: string, contentDir: string) {
  const link = join(docroot, 'wp-content');

  if (existsSync(link)) {
    if (lstatSync(link).isSymbolicLink()) return;
    return;
  }

  const target = relative(dirname(link), contentDir);
  symlinkSync(target, link, 'dir');
}

function ensureDefaultContentFiles(contentDir: string) {
  const packageContentDir = resolve(dirname(fileURLToPath(import.meta.url)), '../../defaults/wordpress/content');

  removeLegacyContentFiles(contentDir);
  copyPackageFile(
    join(packageContentDir, 'mu-plugins/astropress-bridge.php'),
    join(contentDir, 'mu-plugins/astropress-bridge.php'),
  );
  copyPackageDirectory(join(packageContentDir, 'mu-plugins/astropress-bridge'), join(contentDir, 'mu-plugins/astropress-bridge'));
  copyPackageDirectory(join(packageContentDir, 'themes/astropress'), join(contentDir, 'themes/astropress'));
}

function removeLegacyContentFiles(contentDir: string) {
  [
    join(contentDir, 'mu-plugins/vitewp-bridge.php'),
    join(contentDir, 'mu-plugins/vitewp-bridge'),
    join(contentDir, 'themes/vitewp'),
  ].forEach((path) => rmSync(path, { recursive: true, force: true }));
}

function copyPackageFile(source: string, destination: string) {
  if (!existsSync(source)) return;

  mkdirSync(dirname(destination), { recursive: true });
  cpSync(source, destination);
}

function copyPackageDirectory(source: string, destination: string) {
  if (!existsSync(source)) return;

  mkdirSync(dirname(destination), { recursive: true });
  cpSync(source, destination, { recursive: true, force: true });
}
